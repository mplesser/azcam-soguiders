...
;*****************************************************************************
    ...
    TABS    4
;*****************************************************************************
;   Code modified for the 512FT 15 March 2007 - R. Tucker
;   Last change 23Oct07 - RAT - flush (MPL timing)
;*****************************************************************************
...
HOLD_S          EQU     $0005           ; S clock timing, $0005
HOLD_RG         EQU     $0006           ; RG timing, $0006
...
HOLD_ADC        EQU     $000F           ; pre-sample settling (was F)
INIT_NROWS      EQU     $213            ; $200=(512)
INIT_NCOLS      EQU     $238            ; $204=(512)+4
INIT_NFT        EQU     $200            ; $200=512		- RAT	
INIT_NFLUSH     EQU     $400            ; $400=1024			
INIT_NCH        EQU     $1              ;
INIT_VBIN       EQU     $2              ;
INIT_HBIN       EQU     $2              ;
INIT_VSKIP      EQU     $0              ;
...
INIT_USEC       EQU     $C8             ;
INIT_OPCH       EQU     $2              ; 1=CH_A 2=CH_B	- RAT
INIT_SCLKS      EQU     $2              ; 1=LEFT (TOP), 2=RIGHT (BOTTOM)	- RAT
...
INT_L       EQU     $0060               ; reset and first integration
INT_H       EQU     $0068               ; second integration and A/D
...
;   512FT SET DAC VOLTAGES  DEFAULTS:  OD=25V  RD=15.0V  OG=-1V
;   PCLKS=+2V -8V SCLKS=+4V -4V RG=+6V 0V
;*****************************************************************************

            ORG     X:VBIAS
            DC      (DZ-0170)           ; OFFSET_R (5mV/DN)	(-0170)
            DC      (DZ-0170)           ; OFFSET_L		(-0170)
            DC      (DZ+0000)           ; B7		N/U
            DC      (DZ-0200)           ; OG  voltage
            DC      (DZ+0000)           ; B5 (10 mV/DN) N/U
            DC      (DZ+1600)           ; RD	(+13v)
            DC      (DZ+2500)           ; OD_R	(+22v)
            DC      (DZ+2500)           ; OD_L
            ...
            DC      (DZ+0000)           ; IPC+ [V1]			(5v)
            DC      (DZ-0400)           ; RG-  [V2]			(-2v)
            DC      (DZ+1600)           ; RG+  [V3]			(+8v)
            DC      (DZ-1200)           ; S-   [V4]			(-5v)
            DC      (DZ+1200)           ; S+   [V5]			(+5v)
            DC      (DZ-0000)           ; N/U  [V6]			(0v)
            DC      (DZ+0000)           ; N/U  [V7]			(0v)
            DC      (DZ-1800)           ; N/U  [V8]			(0v)
            DC      (DZ+0600)           ; N/U  [V9]			(0v)
            DC      (DZ-1600)           ; P1-  [V10]			(-8v)
            DC      (DZ+0600)           ; P1+  [V11]			(+3v)
            DC      (DZ-1600)           ; P2-  [V12]			(-8v)
            DC      (DZ+0600)           ; P2+  [V13]			(+3v)
            DC      (DZ-1600)           ; P3-  [V14]			(-8v)
            DC      (DZ+0600)           ; P3+  [V15]			(+3v)
...
;                    ||||||||||||||||||||
            ...
            DC  %000000000000010010000011

            ORG X:IPCLKS            ; input clamp
            DC  %000000000000010010010011
            DC  %000000000000010010000011

            ORG X:TCLKS             ; read temp monitor
            DC  %000000000000010010000010
            DC  %000000000000010010000011

            ORG X:PCLKS_FT          ; frame transfer P3-P2-P1-P3
            DC  %000001001000010010000011
            DC  %000011011000010010000011
            DC  %000010010000010010000011
            DC  %000010110100010010000011
            DC  %000000100100010010000011
            DC  %000001101100010010000011

            ORG X:PCLKS_RD          ; parallel transfer P3-P2-P1-P3
            DC  %000000001000010010010011
            DC  %000000011000010010010011
            DC  %000000010000010010000011
            DC  %000000010100010010000011
            DC  %000000000100010010000011
            DC  %000000001100010010000011

; ....................................................................
; original

;            ORG X:PCLKS_FL          ; parallel flush P3-P2-P1-P3
;            DC  %000001001000010010000011
;            DC  %000011011000010010000011
;            DC  %000010010000010010000011
;            DC  %000010110100010010000011
...
; test
            ...
            ORG X:PCLKS_FL          ; parallel flush P3-P2-P1-P3
            DC  %000001001000010010000011
            DC  %000011011000111111100011
            DC  %000010010000111111100011
            DC  %000010110100111111100011
            DC  %000000100100111111100011
            DC  %000001101100010010000011

; ....................................................................

            ORG X:INT_L             ; reset and first integration
            DC  %000000000000010010100011   ; RG ON  FRST ON
            DC  %000000000000010010000011   ; RG OFF
            DC  %000000000000010010000001   ; FRST OFF
            DC  %000000000000010010001001   ; FINT+ ON
            DC  %000000000000010010000001   ; FINT+ OFF

            ORG X:INT_H             ; second integration and A to D
            DC  %000000000000010010000101   ; FINT- ON
            DC  %000000000000010010000001   ; FINT- OFF
            DC  %000000000000010010000000   ; /CONVST ON
            DC  %000000000000010010000001   ; /CONVST OFF
            DC  %000000000000010010100011   ; FRST ON RG ON

            ORG X:SCLKS_R           ; serial shift (right) S2-S3-S1-S2

            DC  %000000000000110110000001	; right
...
;            DC  %000000000000011011000001	;left
;            DC  %000000000000001001000001
;            DC  %000000000000101101000001
;            DC  %000000000000100100000001
...
;            DC  %000000000000010010000001
            ...
            ORG X:SCLKS_L           ; serial shift (left) S2-S1-S3-S2
...
;            DC  %000000000000010010000001
            ...
            ORG X:SCLKS_FLL         ; serial flush (left) S2-S1-S3-S2
            DC  %000000000000011011100011
            DC  %000000000000001001100011
            DC  %000000000000101101100011
            DC  %000000000000100100100011
            ...
            DC  %000000000000110011000001
            DC  %000000000000100001000001
            DC  %000000000000101101000001
            DC  %000000000000001100000001
            DC  %000000000000011110000001
            DC  %000000000000010010000001

            ORG X:SCLKS_FLB         ; serial flush (both)
            DC  %000000000000110011100011
            DC  %000000000000100001100011
            DC  %000000000000101101100011
            DC  %000000000000001100100011
            DC  %000000000000011110100011
            DC  %000000000000010010100011

            ORG X:SCLKS_FF          ; serial flush (fast) DG
            DC  %000000000000010010100011
            DC  %000000000000111111100011
            DC  %000000000000111111100011
            DC  %000000000000111111100011
            DC  %000000000000111111100011   ; dummy code
            DC  %000000000000010010100011   ; dummy code
...
;--------------------------------------------------------------------
            ...
            MOVE    #>$FFFF,X1              ; prepare 16-bit mask   - RAT
            AND     X1,A1                   ; get all 16 bits
